...
$x = scalar( @{ $bach->search( key => 'C_M C#M DbM D_M D#M EbM E_M F_M F#M GbM G_M G#M AbM A_M A#M BbM B_M' ) } );
$y = scalar( @{ $bach->search( key => 'C_m C#m Dbm D_m D#m Ebm E_m F_m F#m Gbm G_m G#m Abm A_m A#m Bbm B_m' ) } );
is $x, 35, 'major keys search';
is $y, 25, 'minor keys search';
is $x + $y, 60, 'major + minor = total keys';
